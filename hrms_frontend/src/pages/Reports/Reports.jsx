...
const Reports = () => {
  ...
  // 1. Employee Report Filtering
  const employeeReportData = useMemo(() => {
    return employees.filter((emp) => selectedDept === "All" || emp.department === selectedDept);
  }, [employees, selectedDept]);

  // 2. Attendance Report Filtering
  ...
  // 3. Leave Report Filtering
  const leaveReportData = useMemo(() => {
    return leaveRequests.filter((req) => {
      const emp = employees.find((e) => e.id === req.employeeId);
      return emp && (selectedDept === "All" || emp.department === selectedDept);
    });
  }, [leaveRequests, employees, selectedDept]);

  // 4. Payroll Report Filtering
  ...
  const handleExport = () => {
    ...
    if (activeTab === "employees") {
      ...
      rows = employeeReportData.map(
        (e) => `"${e.id}","${e.name}","${e.department}","${e.designation}","${e.joiningDate}","${e.status}"`
      );
    } else if (activeTab === "attendance") {
      headers = ["Employee ID,Date,Check In,Check Out,Working Hours,Status"];
      rows = attendanceReportData.map(
        (l) => `"${l.employeeId}","${l.date}","${l.checkIn}","${l.checkOut}","${l.hours}","${l.status}"`
      );
    } else if (activeTab === "leaves") {
      headers = ["Employee Name,Leave Type,Start Date,End Date,Total Days,Status"];
      rows = leaveReportData.map(
        (r) => `"${r.employeeName}","${r.leaveType}","${r.startDate}","${r.endDate}","${r.totalDays}","${r.status}"`
      );
    } else if (activeTab === "payroll") {
      headers = ["ID,Name,Department,Basic Salary,Allowances,Deductions,Net Salary"];
  ...
  const payrollColumns = [
    ...
    { header: "Name", accessor: "name", sortable: true },
    { header: "Department", accessor: "department", sortable: true },
    { header: "Basic Salary", accessor: "payroll.basic", sortable: true, render: (row) => `$${row.payroll?.basic.toLocaleString()}` },
    { header: "Allowances", accessor: "payroll.allowances", sortable: true, render: (row) => `$${row.payroll?.allowances.toLocaleString()}` },
    { header: "Deductions", accessor: "payroll.deductions", sortable: true, render: (row) => `$${row.payroll?.deductions.toLocaleString()}` },
    { header: "Net Salary", accessor: "payroll.net", sortable: true, render: (row) => `$${row.payroll?.net.toLocaleString()}` }
  ];

  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Reports & Analytics" subtitle="Aggregate data export and metrics visualizations" />
      ...
      <div className="bg-bg-secondary border border-border-color rounded-2xl p-6 shadow-sm">
        ...
        <div className="flex justify-between items-center gap-4 mb-6 flex-wrap">
          ...
          <Button variant="outline" iconBefore={<MdFileDownload />} onClick={handleExport}>
            Export Report CSV
          </Button>
        </div>
        ...
        {activeTab === "employees" && (
          <DataTable columns={empColumns} data={employeeReportData} pageSize={5} />
        )}
        {activeTab === "attendance" && (
          <DataTable columns={attColumns} data={attendanceReportData} pageSize={5} />
        )}
        {activeTab === "leaves" && (
          <DataTable columns={leaveColumns} data={leaveReportData} pageSize={5} />
        )}
        {activeTab === "payroll" && (
          <DataTable columns={payrollColumns} data={payrollReportData} pageSize={5} />
        )}
      </div>
    </div>
  );
