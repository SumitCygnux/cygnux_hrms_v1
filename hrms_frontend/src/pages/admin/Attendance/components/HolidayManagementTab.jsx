...
const HolidayManagementTab = () => {
  ...
  const handleDeleteHoliday = async (holiday) => {
    ...
    const result = await Swal.fire({
          title: "Are you sure?",
          text: `Do you want to delete ${holiday.holidayName}?`,
          icon: "warning",
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: "isPaid",
      render: (row) => (
        <Badge status={row.isPaid ? "Active" : "InActive"}>
    ...
    <div className="flex flex-col gap-6">
      ...
      {viewMode === "List" ? (
        <DataTable
          ...
          data={holidays}
          pageSize={8}
          emptyMessage="No holidays registered yet."
        />
      ) : (
        <div className="bg-bg-secondary border border-border-color rounded-2xl p-6 shadow-sm flex flex-col gap-4">
          ...
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex gap-2">
              <button
                onClick={prevMonth}
                className="p-2 border border-border-color rounded-md bg-bg-primary hover:bg-bg-secondary text-text-primary text-xl"
              >
                <MdChevronLeft />
              </button>
              <button
              ...
              return (
                <div
                  ...
                  className={`border border-border-color rounded-lg p-2 flex flex-col items-start gap-1 justify-between min-h-[90px] relative transition-all ${
                    isCurrentMonth
                      ? "bg-bg-primary"
                      : "bg-bg-secondary/40 opacity-40"
                  } ${holiday ? "border-rose-300 bg-rose-50/10 shadow-sm shadow-rose-100" : ""}`}
                >
                  <span
                    className={`text-xs font-bold ${holiday ? "text-rose-600" : "text-text-secondary"}`}
                  >
                    {day.date()}
                  </span>
                  ...
                  {holiday && (
                    <div
                      className="bg-rose-500 text-white rounded px-1.5 py-0.5 text-[9px] font-bold w-full truncate"
