    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
        ...
        <KPICard
          ...
          value={metrics.pendingClockOuts}
          icon={<MdHourglassEmpty />}
          color="rose"
          variant="clean"
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-bg-secondary border border-border-color rounded-2xl p-6 shadow-sm">
          <span className="text-sm font-bold text-text-primary mb-4 block">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-bg-secondary border border-border-color rounded-2xl p-6 shadow-sm">
          ...
          <BarChartComponent
            data={charts.departmentAttendance}
            xKey="name"
            yKey="value"
            color="#6366F1"
            label="Attendance %"
        ...
        <div className="bg-bg-secondary border border-border-color rounded-2xl p-6 shadow-sm">
          <span className="text-sm font-bold text-text-primary mb-4 block">
            Active Shifts Allocation
          </span>
          <PieChartComponent
            data={charts.shiftUtilization}
            nameKey="name"
            valueKey="value"
          />
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-bg-secondary border border-border-color rounded-2xl p-6 shadow-sm flex flex-col gap-4">
          <span className="text-sm font-bold text-text-primary">
            Latest Attendance Requests
          </span>
          <div className="flex flex-col gap-3">
            {recentRequests.map((req) => (
              <div
                key={req.id}
                className="flex justify-between items-center border-b border-border-color pb-2 last:border-b-0 last:pb-0"
              >
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] text-text-secondary">
                    {req.requestType} for {req.requestDate}
                  </span>
                </div>
            ...
            {recentRequests.length === 0 && (
              <span className="text-xs text-text-secondary text-center py-4">
            ...
            {recentAssignments.map((assign) => (
              ...
              >
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] text-text-secondary">
                    {assign.shiftName} (From: {assign.effectiveFrom})
                  </span>
                </div>
                <Badge
                  status={assign.status === "Active" ? "Active" : "InActive"}
                >
                  {assign.status}
                </Badge>
              </div>
