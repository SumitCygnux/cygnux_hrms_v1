...
const Navbar = ({ onMobileToggle }) => {
  ...
  const todayLog = attendanceLogs.find((l) => l.employeeId === currentUser.id && l.date === today);
  ...
  return (
    <header className="h-[70px] sticky top-0 right-0 flex items-center justify-between px-6 bg-bg-secondary/80 backdrop-blur-md border-b border-border-color z-[90] transition-all">
      ...
      <div className="flex items-center gap-5">
        ...
        {/* Theme Toggle */}
        <button className="bg-transparent border-none text-text-secondary text-xl cursor-pointer flex items-center justify-center p-2 rounded-full relative transition-all hover:bg-bg-primary hover:text-primary" onClick={toggleTheme} aria-label="Toggle Theme">
